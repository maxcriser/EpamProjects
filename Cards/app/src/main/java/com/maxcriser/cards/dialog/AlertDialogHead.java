...
public abstract class AlertDialogHead<T extends AlertDialogHead> {
    ...
    public AlertDialogHead(final Context context) {
        init(new AlertDialog.Builder(context));
    }

    public AlertDialogHead(final Context context, final int theme) {
        init(new AlertDialog.Builder(context, theme));
    }

    private void init(final AlertDialog.Builder dialogBuilder) {
    ...
    public T setMessage(@StringRes final int message) {
        return setMessage(string(message));
    }

    public T setMessage(final CharSequence message) {
    ...
    public T setTopTitle(@StringRes final int title) {
        return setTopTitle(string(title));
    }

    public T setTitle(final CharSequence title) {
        titleView.setVisibility(View.VISIBLE);
        titleView.setText(title);
        return (T) this;
    }

    public T setTopTitle(final CharSequence title) {
        topTitleView.setVisibility(View.VISIBLE);
        topTitleView.setText(title);
        return (T) this;
    }

    public T setTopTitleColor(final int color) {
        topTitleView.setTextColor(color);
        return (T) this;
    }

    public T setIcon(final Bitmap bitmap) {
        iconView.setVisibility(View.VISIBLE);
        iconView.setImageBitmap(bitmap);
        return (T) this;
    }

    public T setIcon(final Drawable drawable) {
        iconView.setVisibility(View.VISIBLE);
        iconView.setImageDrawable(drawable);
        return (T) this;
    }

    public T setIcon(@DrawableRes final int iconRes) {
        iconView.setVisibility(View.VISIBLE);
        iconView.setImageResource(iconRes);
        return (T) this;
    }

    public T setIconTintColor(final int iconTintColor) {
        iconView.setColorFilter(iconTintColor);
        return (T) this;
    }

    public T setTitleGravity(final int gravity) {
        titleView.setGravity(gravity);
        return (T) this;
    }

    public T setMessageGravity(final int gravity) {
        messageView.setGravity(gravity);
        return (T) this;
    }

    public T setTopColor(@ColorInt final int topColor) {
        findView(R.id.ld_color_area).setBackgroundColor(topColor);
        return (T) this;
    }

    public T setTopColorRes(@ColorRes final int topColoRes) {
     ...
     * ic_drawer_help you in this.
     */
    public T setInstanceStateHandler(final int id, final AlertHandler handler) {
        handler.handleDialogStateSave(id, this);
        return (T) this;
    }

    public T setCancelable(final boolean cancelable) {
        dialog.setCancelable(cancelable);
        return (T) this;
    }

    public T setSavedInstanceState(final Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            final boolean hasSavedStateHere =
    ...
    protected String string(@StringRes final int res) {
        return dialogView.getContext().getString(res);
    }

    protected int color(@ColorRes final int colorRes) {
    ...
    protected <ViewClass extends View> ViewClass findView(final int id) {
    ...
    protected class ClickListenerDecorator implements View.OnClickListener {
        ...
        protected ClickListenerDecorator(final View.OnClickListener clickListener, final boolean closeOnClick) {
        ...
        public void onClick(final View v) {
            if (clickListener != null) {
                if (clickListener instanceof AlertDialogGap.DialogOnClickListenerAdapter) {
                    final AlertDialogGap.DialogOnClickListenerAdapter listener =
